...
Sky is a game engine used for researching GPU driven rendering and their application in games. The goal is to provide a fast way to experiment with new graphics techniques and algorithms in real-time applications. Sky is written in Java, which makes cross-platform support easier compared to C++. Rendering is supported through Vulkan and shaders are written in Slang, although any language using SPIR-V is natively supported.
